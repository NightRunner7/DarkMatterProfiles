...
# ##################################### PLOTTING COMPARISON ########################################################## #
def rescale_time(model, _sigma_eff, beta=0.85):
...
if make_gilman_plot:
    ...
    # --- M200 = 10^7 Msun ---
    t_lo1, rho_lo1 = gravoEvolution_RSIDM_1.return_rho_core_evolution(elements=elements, index=2)
    t_scale = t_lo1 / t_lo1[-1]
    halo_rSIDM = TruncatedNFWProfile(_M_vir=10**M_power_arr[0], _con=con_arr[0])
    tau_kappa = halo_rSIDM.tau(beta=0.85, sigma_eff=sigma_m_arr[0])
    ax.plot(
        # t_scale, rho_lo1,
    ...
    tau_kappa = halo_rSIDM.tau(beta=0.85, sigma_eff=sigma_m_arr[4])
    ax.plot(
        t_scale, rho_lo1,
        ls='-', lw=2.4, color=color_3,
        label=r"$M_{200}=10^{9}\,M_\odot$"
    )
    # Gilman results
    # ax.plot(
