...
z = 0;
Mi = 11.30103;(*match the range in "Evolution"*)
Mf = 10.9;
NM = 1;
\[Delta]M = (Mf - Mi)/NM;
...
(**)
Mi = 11.30103;(*to scan halo masses, change also "p" range \
...
equilibrium profile*)
  \[Xi]array = 
   Table[(*\[Xi]=*)
    1000.0(*10^\[Sigma]mRSIDMint[Log10[\[Sigma]array[[j]]*r0/
  ...
  If[(*\[Rho]array[[1]]>101.*)
   Log10[t] > 
    Log10[10.0],(*condition to finish computation*)
   Do[\[Rho]sol[Tticker][j] = \[Rho]array[[j + 1]], {j, 0, NR + 1}];
 ...
 data\[Rho]solfinal = 
  Flatten[Table[{T[i], rsol[i][j], \[Rho]sol[i][j]}, {i, 0, 
     Tticker}, {j, 0, NR + 1}], 1];
 Export["\[Rho]sol_M_" <> ToString[11.30103] <> "_t_" <> 
    ToString[10.0] <> "_sigma_" <> ToString[1000.0] <> 
    ".txt", data\[Rho]solfinal, "Table"];
 data\[Sigma]solfinal = 
  Flatten[Table[{T[i], rsol[i][j], \[Sigma]sol[i][j]}, {i, 0, 
     Tticker}, {j, 0, NR + 1}], 1];
 Export["\[Sigma]sol_M_" <> ToString[11.30103] <> "_t_" <> 
    ToString[10.0] <> "_sigma_" <> ToString[1000.0] <> 
