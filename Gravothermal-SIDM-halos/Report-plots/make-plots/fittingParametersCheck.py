...
was set in paper 2205.02957. We will gona investigate how changes in time and so on.
...
def evolutionRadiusCore_Model(_mvir, _sigma_m, _beta, _max_time,
    ...
    plt.title(f'Evolution of radius in fitting. Mass=10^{_power_mass}, sigma={_sigma_m}, time={_max_time}',
              fontsize=18)
    ...
    plt.axvline(x=trans_dimless_T, color='black',
                label='Fitting late-time is started,' + "\n" +
                      f'ocured in: {"{:.2f}".format(trans_dimless_T)} [dimless].')

    # ax.tick_params(axis='both', which='major', labelsize=12)
    # --- LEGEND AND SAVE
    ax.legend(fontsize=14)
    plt.savefig(_path + '/' + 'Evolution_radius_fitting' + '.png', dpi=300)
...
def differenceWtithNFW(_mvir, _sigma_m, _beta, _max_time,
                        _path,
                        _z=0, plot_show=False):
    """
    ...
    fig, ax = plt.subplots(figsize=(11.0, 7.0))
    plt.xlabel(r'$\hat{t} \ \left[\frac{4}{\sqrt{\pi}} \hat{\sigma}_{m} \hat{\nu} \hat{\rho} \right]$', fontsize=18)
    # plt.ylabel(r'$\rho_{core} \, r_{core}^{s} \, r_{out}^{3-s}$', fontsize=18)
    ...
    plt.axhline(y=1.0, color='violet', linestyle=':',
                label='physically proper value of coefficient,')
    plt.axvline(x=trans_dimless_T, color='grey', linestyle='dashdot',
                label='Fitting late-time is started,' + "\n" +
                      f'from: {"{:.2f}".format(trans_dimless_T)} [dimless].')
    # ax.set_yscale('log')
    ax.legend(fontsize=14)
