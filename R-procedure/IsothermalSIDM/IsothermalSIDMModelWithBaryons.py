...
Isothermal SIDM Model / Isothermal profile. In this code baryons can be implemented, but main
class (`IsoEvolutionWithBaryons`) in the code does not have autonomization in such case.
...
def deltaSqare(p, rhob0, r0, rhoCDM1, MCDM1, r):
    """
    ...
    Syntax:
        ...
        deltaSqare(p,rho1,M1,r0,rhoCDM1,MCDM1,r)
    ...
    where
        ...
        rhoCDM1: CDM density to match at r_1 [M_sun/kpc^3] (float)
        MCDM1: CDM enclosed mass to match at r_1 [M_sun] (float)
    ...
    # M = aux.Miso_logspaceR(r, rho)  # should be enough accurate
    # calculate delta^2
...
def stitchSIDM(r1, halo, N=500):
    ...
    # large baryon size
    rhob0 = 1e-6  # [m_sun/kpc^3] #lets try with 1e-9
    r0 = 100.  # [kpc]

    # Set the options for the Powell method
    name_method = 'Powell'  # 'Powell', 'L-BFGS-B'
    options = {
        # 'ftol': 1e-6, 'maxiter': 10000, 'xtol': 1e-4
    }
...
def findTwoDeltaMinimum(r1, halo, N=500, r_low=-3.0):
    """
    Find the two minimum (in two different region), which occured in plot log10(delta). This function
    using the isothermal SIDM core, which is stitched smoothly to the CMD-like outskirt.
    Important note!: this works only for cose: CDM-only.

    Syntax:

        .findTwoDeltaMinimum(r1,h,N=500)
    ...
    where
        ...
        N: length of the isothermal-core profile to be returned
           (int, default=500)
        r_low: we will consider: r = np.logspace(r_low, np.log10(r1), N),
               so its the way to set from what radius we start calculating.
               (float, default=3.)
    ...
    Return:
        ...
        for both case: high density and low density. We return this as a list: [data_lowDens, data_higDens]
    """
    ...
    res = minimize(deltaSqare, np.array([lgrhodm0_init, lgsigma0_init]),
                   args=(rhob0, r0, rhoCDM1, MCDM1, r),
                   bounds=((lgrhodm0_lo, lgrhodm0_hi), (lgsigma0_lo, lgsigma0_hi)),
                   method=name_method, options=options)
    rhodm0 = 10.**res.x[0]
    sigma0 = 10.**res.x[1]
    a = cfg.FourPiG * r0**2 * rhodm0 / sigma0**2
    b = cfg.FourPiG * r0**2 * rhob0 / sigma0**2
    fun_h = h(r/r0, a, b)
    rho = rhodm0 * np.exp(fun_h)
    M = aux.Miso_simpson(r, rho)
    Vc = np.sqrt(cfg.const_G_starUnits*M/r)
    # register
...
# ######################### ISOTHERMAL CLASS ######################### #
class IsoEvolutionWithBaryons(object):
    """
    Find the isothermal SIDM core that is stitched smoothly to the
    ...
    def __init__(self, _halo, r1, nr=500, cff=0.001, rel_err_mergin=1.0 + 40e-2):
        ...
        self.r1 = r1  # [kpc]
        # Baryon properties - here we consider DM-only case, so we
        # use an arbitrarily low value for baryon density and an arbitrarily
        # large baryon size
        self.rhob0 = 1e-6  # 1e-6, 326502405.13772416  # [M_sun/kpc^3]
        self.r0 = 100.0  # 100.0, 0.7870057685088806  # [M_sun]
    ...
    def initial_guess(self, nr=500, r_low=-4.0, initSetUp=True):
        ...
        lg_sigma0_lo = np.log10(0.5 * self.sigmaCDM1)
        ...
        res = minimize(deltaSqare, np.array([lg_rho0_init, lg_sigma0_init]),
                       args=(self.rhob0, self.r0, self.rhoCDM1, self.MCDM1, r),
    ...
    def new_evolution_step_naive(self, r1, nr=500, r_low=-4.0, initSetUp=True):
        ...
        lgrhodm0_hi = np.log10(self.rhoCDM1 * 1e3)  # <<< test: upper bound
        ...
        # lgrhodm0_hi = np.log10(1e1 * self.rhoCDMres)  # <<< test: upper bound
        lgrhodm0_hi = np.log10(2e1 * self.rhoCDMres)  # <<< test: upper bound
    ...
    def prepare_ISO_data(self, _r, rho0, sigma0):
        ...
        """
        # prepare
        a = cfg.FourPiG * self.r0 ** 2 * rho0 / sigma0 ** 2
        b = cfg.FourPiG * self.r0 ** 2 * self.rhob0 / sigma0 ** 2
        fun_h = h(_r / self.r0, a, b)
        # data
        rho = rho0 * np.exp(fun_h)  # [M_sun/kpc^3] density profile
        # M = aux.Miso_simpson(_r, rho)  # [M_sun] enclosed mass
        M = aux.Miso_logspaceR(_r, rho)  # [M_sun] enclosed mass
