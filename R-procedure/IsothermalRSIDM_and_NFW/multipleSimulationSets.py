...
sigma_m = 200.0  # [cm^2/g] annihilation cross-section
sigma_m_SI = sigma_m * 10**(-4) * 10**3  # [m^2/kg]
M_vir = 1*10**11.0  # [M_sun] Viral mass
...
time_tilda_start = 6*10**(-3)  # [dimensionless]
...
tage_tilda_to_plot = [3.0, 20.0, 40.0, 50.0, 100.0, 170.0, 330.9]  # [dimensionless]
...
for time in range(0, diff_tim):
    ...
    if ((len(roots)==1) and (counter_jump<3)):
