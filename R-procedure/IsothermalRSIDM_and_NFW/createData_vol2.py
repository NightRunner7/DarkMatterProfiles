...
# ################################################# RSIDM PROFILE #################################################### #
sigma_eff_from_nu_kpcGyr, sigma_eff_from_nu_kms, _grid = build_sigma_m_eff_p5_interpolator(
    sigma_m0=0.008,
    m_GeV=0.02,
    L=0,
    Gamma=6e-12,
...
sigma_m = 17.65  # [cm^2/g] annihilation cross-section
sigma_m_SI = sigma_m * 10**(-4) * 10**3  # [m^2/kg]
M_vir = 1*10**10.5  # [M_sun] Viral mass
const_c = 10.0  # [dimensionless] concentration of DM
...
# print("sigma_m * rho_s * r_s", sigma_m*rounded_rho_s*r_s)

# ################################### TIME AND SPACE: USER SETS! ################################### #
# dimensional time
...
r_1, roots, dbg = find_all_r1_roots_rsidm(
    halo=NFW_profile,
    tage_gyr=tage_grid[0],
...
if LowDense is True:
    file_name = f'RSIDM_M_{format_log10mass}_t_{format_t}_sigma_{sigma_m}_con_{format_c}.csv'
else:
    file_name = f'RSIDM_HiDens_M_{format_log10mass}_t_{format_t}_sigma_m_{sigma_m}_con_{format_c}.csv'
...
def smooth_and_plot_rho_vs_time(
    t,
    rho,
    n_fine=800,
    loglog=True,
    merge_duplicates="median",  # "median", "mean", or None
    xlabel=r"$t$",
    ylabel=r"$\rho_c(t)$",
    title="Central density evolution"
):
    """
    Make rho(t) continuous with shape-preserving interpolation (PCHIP)
    and plot points + smooth curve.

    Parameters
    ----------
    t : array-like
        Time values (can be linear or log10-time; just be consistent).
    rho : array-like
        Density values (must be >0 if loglog=True).
    n_fine : int
        Number of points in the smooth curve.
    loglog : bool
        If True, do interpolation in log10(rho) vs (t) and plot log-log.
        Use this if rho spans orders of magnitude.
    merge_duplicates : str or None
        If you have repeated identical time stamps, merge them by median/mean.
    """
    ...
    # keep finite
    m = np.isfinite(t) & np.isfinite(rho)
    t, rho = t[m], rho[m]

    # if loglog, need rho>0
    if loglog:
        m = rho > 0
        t, rho = t[m], rho[m]
    ...
    # merge duplicate times if needed (PCHIP wants strictly increasing x)
    if merge_duplicates is not None:
        uniq_t = []
        uniq_rho = []
        i = 0
        while i < len(t):
            ...
            while j < len(t) and t[j] == t[i]:
                j += 1
            vals = rho[i:j]
            if merge_duplicates == "median":
                uniq_t.append(t[i])
                uniq_rho.append(np.median(vals))
            elif merge_duplicates == "mean":
                uniq_t.append(t[i])
                uniq_rho.append(np.mean(vals))
            else:
                raise ValueError("merge_duplicates must be 'median', 'mean', or None")
            i = j
        t = np.asarray(uniq_t)
        rho = np.asarray(uniq_rho)

    # require at least 2 points
    if len(t) < 2:
        raise ValueError("Need at least 2 valid (t, rho) points to interpolate.")

    # build smooth curve
    t_fine = np.linspace(t.min(), t.max(), n_fine)

    if loglog:
        # shape-preserving interpolation in log10(rho)
        y = np.log10(rho)
        interp = PchipInterpolator(t, y, extrapolate=False)
        rho_fine = 10**interp(t_fine)
    else:
        # direct interpolation in rho (also fine if rho doesn't span many decades)
        interp = PchipInterpolator(t, rho, extrapolate=False)
        rho_fine = interp(t_fine)

    # plot
    plt.figure(figsize=(7.2, 4.8))

    if loglog:
        # plt.loglog(t, rho, "o", ms=4, label="data")
        plt.loglog(t_fine, rho_fine, "-", lw=2, label="RSIDM")
    else:
        plt.plot(t, rho, "o", ms=4, label="data")
        plt.plot(t_fine, rho_fine, "-", lw=2, label="PCHIP smooth")
...
# -------------------------
t_list  = time_tilda_l
rho_list = rho0_LoDens_tilda_l

def hampel_filter_logy(t, rho, k=3, nsig=3.0):
    # Hampel filter on log10(rho)
    t = np.asarray(t, float)
    rho = np.asarray(rho, float)
    m = np.isfinite(t) & np.isfinite(rho) & (rho > 0) & (t > 0)
    t, rho = t[m], rho[m]
    ...
    for i in range(len(y)):
        ...
        mad = np.median(np.abs(y[lo:hi] - med)) + 1e-12
        if np.abs(y[i] - med) > nsig * 1.4826 * mad:
            y_f[i] = med  # replace outlier by local median

    return t, 10**y_f

t_clean, rho_clean = hampel_filter_logy(t_list, rho_list, k=3, nsig=3.0)

smooth_and_plot_rho_vs_time(t_clean, rho_clean, loglog=True)
